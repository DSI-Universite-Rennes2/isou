...
function cron_rotate_database() {
	...
	if(!is_file(PRIVATE_PATH.'/database/isou-'.($year-1).'.sqlite3')){
		...
		// Ferme tous les évènements en cours.
		$sql = "UPDATE events SET enddate = ? WHERE enddate IS NULL";
		$query = $DB->prepare($sql);
		$commit &= $query->execute(array($oldyear));

		if(copy(PRIVATE_PATH.'/database/isou.sqlite3', PRIVATE_PATH.'/database/isou-'.($year-1).'.sqlite3')){
			// Supprime tous les anciens évènements dont la date est inférieure au 1er janvier N+1.
			$sql = "DELETE FROM events WHERE enddate < ?";
			$query = $DB->prepare($sql);
			$commit &= $query->execute(array($oldyear));

			// Initialise tous les évènements à la nouvelle année.
			$sql = "UPDATE events SET begindate = ?, enddate = NULL";
			$query = $DB->prepare($sql);
			$commit &= $query->execute(array($newyear));
		}else{
		...
		$sql = "UPDATE configuration SET value=? WHERE key=?";
		$query = $DB->prepare($sql);
		$commit &= $query->execute(array(TIME, 'last_yearly_cron_update'));
