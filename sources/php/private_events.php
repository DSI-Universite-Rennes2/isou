...
if(count($_POST) > 0){
	...
	$sql = "SELECT E.idEvent, E.beginDate, E.endDate, EI.period, EI.idEventDescription, D.description, EI.isScheduled, S.idService, S.name, S.nameForUsers".
			...
			if ($_SESSION['hide'] === 1){
				$sql .= " AND (E.endDate - E.beginDate > :4))";
				$param = array(TIMESTAMP_OF_72H_BEFORE_TODAY, TIMESTAMP_OF_LAST_CALENDAR_DAY, TIMESTAMP_OF_72H_BEFORE_TODAY, TIMESTAMP_OF_LAST_CALENDAR_DAY, TOLERANCE);
			}else{
				$sql .= ")";
