<div id="content">
	...
	{if count($forcedservices) > 0}
		...
		{foreach $forcedservices as $forcedservice}
			<li>
				<img src="{$smarty.const.URL}/images/{$STATES.{$forcedservice->state}->src}" alt="{$STATES.{$forcedservice->state}->alt}" />
	...
	{if $smarty.get.type == 0}
		...
		{include file="events/view_unscheduled.tpl"}
		{include file="events/add.tpl"}
	{else if $smarty.get.type == 2}
		...
		{include file="events/view_regular.tpl"}
		{include file="events/add.tpl"}
	{else if $smarty.get.type == 3}
		...
		{include file="events/view_closed.tpl"}
		{include file="events/add.tpl"}
